...
const F = {
  ...
  PROD_TIPO:       "fldD3rVmwS7K8C3YD",
  PROD_NOTAS:      "fldPe90fLptIy0PT1",
...
function guardarToken(){
  ...
  actualizarBadge();
  // Cada pantalla nombra distinto su carga inicial
  if(typeof iniciar === 'function') iniciar();
  else if(typeof cargarTodo === 'function') cargarTodo();
}

// El id del badge cambia segun la pantalla, por eso se aceptan los dos
function actualizarBadge(){
  const b = document.getElementById('badge-conexion') || document.getElementById('badge');
...
// Dias transcurridos desde una fecha, para marcar pedidos atrasados
...
/*
  ...
  Tiene dos modos:
  - Sin lista de campos: trae todos, identificados por ID de campo. Lo usa el
    catalogo. Filtrar con fields[] por ID es fragil: si un ID no coincide, la
    respuesta llega vacia sin ningun error, que fue justo el bug de los
    productos "undefined".
  - Con lista de campos: filtra por NOMBRE y los devuelve por nombre. Lo usan
    las pantallas de operacion y panel de control.
*/
async function traerTodo(tabla, campos){
  ...
  do {
    const q = `?pageSize=100${filtro}` + (offset ? '&offset=' + offset : '');
...
/*
  SUBIR UNA FOTO COMO ADJUNTO
  Los adjuntos no van por la API normal: usan otro dominio y reciben el
  archivo en base64. Se usa al dar de alta un color nuevo.
*/
async function subirAdjunto(registroId, campoId, base64ConPrefijo, nombreArchivo){
  const soloBase64 = base64ConPrefijo.split(',')[1];
  const url = `https://content.airtable.com/v0/${BASE_ID}/${registroId}/${campoId}/uploadAttachment`;
  const r = await fetch(url, {
    method: 'POST',
    headers: {
      'Authorization': 'Bearer ' + getToken(),
  ...
  if(!r.ok){
    const err = await r.json().catch(()=>({}));
    throw new Error(err.error?.message || ('No se pudo subir la foto (' + r.status + ')'));
  }
  return r.json();
}
...
/*
  NOMBRE DE ARCHIVO PARA GOOGLE DRIVE
  En Drive hay una carpeta por variante: /<Producto>/<Color>/. Los archivos se
  nombran "<Producto> <COLOR>.jpg" para que el orden alfabetico agrupe bien y
  para que el nombre coincida con la carpeta donde va.
*/
function nombreArchivoDrive(producto, color, sufijo){
  const limpio = t => t.trim().replace(/[\\/:*?"<>|]/g,'');
  return `${limpio(producto)} ${limpio(color).toUpperCase()}${sufijo ? ' ' + sufijo : ''}.jpg`;
}
